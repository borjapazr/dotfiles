...
dot::list_bash_files() {
  grep '#!/usr/bin/env bash' "$DOTFILES_PATH"/{bin,scripts,dots/shell,installer} -R --exclude-dir=.zim | awk -F':' '{print $1}'
  find "$DOTFILES_PATH"/{bin,scripts,dots/shell} -not -path "*/.zim/*" -type f -name "*.sh"
}
